...
#define RXE_IB_METHOD_SUSPEND_VHCA_LOCAL	((1u << RXE_UVERBS_ID_NS_SHIFT_LOCAL) + 6)
...
#define RXE_IB_ATTR_SUSPEND_VHCA_SUSPEND_LOCAL (1u << RXE_UVERBS_ID_NS_SHIFT_LOCAL)
static bool rxe_vhca_suspended;
...
{
	if (rxe_vhca_suspended && rxe_dump_control_fd >= 0) {
		uint8_t suspend = 0;
		struct {
		...
		cmd.hdr.method_id = RXE_IB_METHOD_SUSPEND_VHCA_LOCAL;
...
static int rxe_suspend_vhca(int fd)
{
	uint8_t suspend = 1;
	struct {
		struct ib_uverbs_ioctl_hdr hdr;
