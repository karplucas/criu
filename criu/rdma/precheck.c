 ...
 *      "is this context held by the snapshot tree?" -- the cross-tree
 *      exclusivity check below uses the same oracle to look at the
 *      *outside* set instead.
 *
...
static int dump_cov_cb(const struct rdma_nl_ctx_info *info, void *arg)
{
	...
	if (!pid_in_tree(cov->tree_pids, cov->n_tree_pids, info->pid)) {
		pr_debug("ctx pid=%d ibdev=%s ctxn=%u outside snapshot tree, skipping (handled by cross-tree "
			 "exclusivity)\n",
 ...
 *      host. We materialise all tuples up front rather than try to do
 *      the analysis incrementally inside the iterator callback; the
 ...
 *      API will close that window; for now we document and accept it
 *      -- the same window already exists for the per-fd dump path's
 *      sysfs probes, so the cross-tree check isn't introducing a new
 *      class of race, just inheriting an existing one.
 */
...
static int cross_tree_collect_cb(const struct rdma_nl_ctx_info *info, void *arg)
{
	...
	if (cc->n_tuples >= cc->cap) {
		...
		nt = xrealloc(cc->tuples, newcap * sizeof(struct cross_tree_tuple));
		if (!nt) {
			cc->oom = 1;
			return -ENOMEM;
		}
	...
	t->pid = info->pid;
	snprintf(t->ibdev, sizeof(t->ibdev), "%.*s", (int)(sizeof(t->ibdev) - 1), info->ibdev);
	return 0;
}

static bool tuple_pid_in_tree(const struct cross_tree_collect_ctx *cc, pid_t pid)
{
	size_t i;
...
int rdma_check_cross_tree_exclusivity(struct pstree_item *root)
{
	...
	if (ret < 0 || cc.oom) {
		pr_err("cross-tree RDMA exclusivity check failed at netlink layer (%d). Failing closed.\n", ret);
		xfree(pids);
		xfree(cc.tuples);
		return -1;
	}

	pr_debug("cross-tree exclusivity: %zu (pid, ibdev) tuple(s) on host, %zu pid(s) in snapshot tree\n",
		 cc.n_tuples, n);

	/*
	 * O(N^2) over collected tuples. Real systems have a handful of
	 * ibdevs and at most low hundreds of contexts; not worth a proper
	 * hashmap until we see this in a profile.
	 */
	ret = 0;
